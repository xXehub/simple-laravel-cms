...
class PageController extends Controller
{
    ...
    public function store(StorePageRequest $request)
    {
        ...
        if ($request->has('builder_data') && !empty($request->input('builder_data'))) {
            // This is a builder page
            $data['builder_data'] = $request->input('builder_data');
        } else {
            // This is a template-based page - ensure builder_data is null or empty
        ...
        return DataTables::of($query)
            ...
            ->addColumn('page_type', function ($page) {
                ...
                if (!empty($page->builder_data)) {
                    // If builder_data is a string, try to decode it
    ...
    public function createBuilder(Request $request)
    {
        ...
        $data['content'] = ''; // Start with empty content for builder pages
        $data['template'] = null; // Builder pages don't use templates
        $data['is_published'] = $request->boolean('is_published', false);
        $data['sort_order'] = 0;
        // Initialize with a default builder structure to mark this as a builder page
