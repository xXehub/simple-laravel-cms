...
<div class="d-flex flex-column h-100 bg-white border-start" id="property-editor">
    ...
    <div class="flex-fill overflow-auto p-3" id="editor-content">
    ...
    <div class="border-top bg-white p-3" id="editor-actions" style="display: none;">
        <div class="d-flex gap-2">
            <button class="btn btn-outline-secondary btn-sm flex-fill" id="reset-properties">
                <i class="ti ti-refresh me-1"></i>
                Reset
            </button>
            <button class="btn btn-outline-primary btn-sm flex-fill" id="apply-properties">
                <i class="ti ti-check me-1"></i>
                Apply
            </button>
            <button class="btn btn-primary btn-sm flex-fill" id="apply-and-close">
                <i class="ti ti-check me-1"></i>
                Save
            </button>
        </div>
...
<template id="property-group-template">
    <div class="property-group mb-3">
        <div class="card shadow-sm">
            <div class="card-header py-2 cursor-pointer property-group-header">
                <div class="d-flex align-items-center">
                    <h6 class="mb-0 property-group-title"></h6>
                    <div class="ms-auto">
                        <i class="ti ti-chevron-down group-toggle text-muted"></i>
                    </div>
                </div>
            </div>
            <div class="collapse show property-group-content">
...
document.addEventListener('DOMContentLoaded', function() {
    const PropertyEditor = {
        ...
        bindEvents() {
            ...
            // Bind action events when actions are shown
            this.bindActionEvents();
        },
        
        bindActionEvents() {
            // Use setTimeout to ensure elements are available and avoid errors
            setTimeout(() => {
                const resetBtn = document.getElementById('reset-properties');
                const applyBtn = document.getElementById('apply-properties');
                const applyCloseBtn = document.getElementById('apply-and-close');
                
                if (resetBtn && !resetBtn.hasAttribute('data-bound')) {
                    resetBtn.addEventListener('click', this.resetProperties.bind(this));
                    resetBtn.setAttribute('data-bound', 'true');
                }
                
                if (applyBtn && !applyBtn.hasAttribute('data-bound')) {
                    applyBtn.addEventListener('click', this.applyProperties.bind(this));
                    applyBtn.setAttribute('data-bound', 'true');
                }
                
                if (applyCloseBtn && !applyCloseBtn.hasAttribute('data-bound')) {
                    applyCloseBtn.addEventListener('click', this.applyAndClose.bind(this));
                    applyCloseBtn.setAttribute('data-bound', 'true');
                }
            }, 100);
        },
        ...
        createPropertyGroup(groupName, properties) {
            ...
            const content = groupElement.querySelector('.card-body');
        ...
        show() {
            // Property editor is always visible, no need to show/hide
        },
        
        hide() {
            // Property editor is always visible, just clear selection
            this.currentComponent = null;
        },
        
        close() {
            // Just deselect component, don't hide editor
            this.currentComponent = null;
