<x-layout.app title="Menus Management - Panel Admin" :pakai-sidebar="true">
    ...
    @push('scripts')
        <script>
            ...
            DataTableGlobal.waitForLibraries().then(() => {
                ...
                function waitForTomSelect() {
                    ...
                    initTomSelect();
                    initializeMenusTable();
                }
                waitForTomSelect();
            });

            function initializeMenusTable() {
                // Menu-specific DataTable configuration
                const menusTableConfig = {
                    ajax: {
                ...
                // Initialize DataTable using global component
                DataTableGlobal.initializeDataTable('#menusTable', {
                    ...
                    document.addEventListener('click', e => {
                        if (e.target.closest('[data-bs-dismiss="modal"]')) hideModal(e.target.closest(
                            '.modal'));
                        if (e.target.classList.contains('modal-backdrop')) document.querySelectorAll(
                                '.modal.show')
                            .forEach(hideModal);
                    });
